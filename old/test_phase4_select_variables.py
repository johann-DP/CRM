...
from phase4 import select_variables


def test_phase4_select_variables_basic():
    df = pd.DataFrame({
        "Code": [1, 2, 3],
    ...
    assert "Type opportunité" not in qual_vars
...
def test_phase4_select_variables_rare_modalities():
    df = pd.DataFrame({
...
def test_phase4_select_variables_constant_numeric_and_text():
    df = pd.DataFrame({
        "ID": [1, 2, 3, 4],
        "amount": [10, 20, 30, 40],
        "const": [1, 1, 1, 1],
