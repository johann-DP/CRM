...
def test_prepare_data_basic():
    ...
    cleaned = pf.prepare_data(df, exclude_lost=True)
...
def test_prepare_data_flag_file(tmp_path: Path):
    ...
    cleaned = pf.prepare_data(df, exclude_lost=False, flagged_ids_path=flagged_path)
    assert list(cleaned["Code"]) == [1, 3]
...
def test_prepare_data_imputes_missing_values():
    ...
    cleaned = pf.prepare_data(df, exclude_lost=False)
