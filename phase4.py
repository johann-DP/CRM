# !/usr/bin/env python3
"""Unified Phase 4 pipeline executing all steps in one pass."""
...
def _load_data_dictionary(path: Optional[Path]) -> Dict[str, str]:
    """Return a mapping from raw column names to cleaned names."""
...
def load_datasets(config: Dict[str, Any]) -> Dict[str, pd.DataFrame]:
    """Load raw and cleaned datasets described in ``config``."""
...
def _run_single_dataset(
    ...
    *,
    ...
    """Execute the analysis pipeline on ``df``."""
    ...
    if not factor_results and not valid_nonlin:
        ...
        return {"metrics": empty, "figures": {}, "df_active": df_active,
                "quant_vars": quant_vars, "qual_vars": qual_vars}
...
def run_pipeline(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if config.get("run_cv"):
        cv_metrics = unsupervised_cv_and_temporal_tests(
            result["df_active"],
            result["quant_vars"],
            result["qual_vars"],
        ...
        cv_metrics.to_csv(output_dir / "cv_metrics.csv")
    ...
    if config.get("compare_versions"):
        ...
        comp["metrics"].to_csv(output_dir / "metrics_versions.csv", index=False)
        result["comparison"] = comp

    tables = {"metrics": result["metrics"]}
    if "cv_metrics" in result:
        tables["cross_validation"] = result["cv_metrics"]
    if "comparison" in result:
        tables["comparison_metrics"] = result["comparison"]["metrics"]

    pdf_path = output_dir / "phase4_report.pdf"
    export_report_to_pdf(result["figures"], tables, pdf_path)
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    parser = argparse.ArgumentParser(description="Unified Phase 4 pipeline")
