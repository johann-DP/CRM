"""Data loading utilities for the CRM pipeline.

This module provides a self-contained ``load_datasets`` function that
reads the raw and cleaned CSV/Excel files specified in a configuration
dictionary.  The implementation is copied from ``phase4v3.py`` so that
...
# Optional global configuration used when ``load_datasets`` is called
# without an explicit mapping.
CONFIG: Dict[str, Any] = {}


def _read_dataset(path: Path) -> pd.DataFrame:
    """Return a DataFrame from ``path`` with basic type conversions."""
    if not path.exists():
...
def _load_data_dictionary(path: Optional[Path]) -> Dict[str, str]:
    """Return a mapping of original column names to cleaned names."""
...
def load_datasets(config: Optional[Mapping[str, Any]] = None) -> Dict[str, pd.DataFrame]:
    """Load datasets defined in ``config`` and return them as a dictionary."""
    ...
    logger.info("Raw dataset loaded from %s [%d rows, %d cols]", raw_path, datasets["raw"].shape[0], datasets["raw"].shape[1])
    ...
    for key, cfg_key in [
        ...
        if not path.exists():
            ...
            continue
        df = _apply_mapping(_read_dataset(path))
        datasets[key] = df
        logger.info(
